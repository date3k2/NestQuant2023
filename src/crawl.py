...
class Crawler(Base):
    def __init__(self, api_key):
        ...
        self._get_download_link_url = (
            Crawler.NESTQUANT_API_ENDPOINT
            + "data/api/download_link?category=%s&symbol=%s&api_key="
            + self._api_key
        )
        self._get_lastest_data_url = (
            Crawler.NESTQUANT_API_ENDPOINT
            + "data/api/lastest?category=%s&symbol=%s&api_key="
            + self._api_key
        )

    def _get_data_response(
        self, category: str, symbol: str, get_historical_data: bool
    ) -> requests.Response:
        """
    ...
    def download_historical_data(self, category: str, symbol: str, location: str):
        """
        ...
        data_response = self._get_data_response(
            category, symbol, get_historical_data=True
        )
        location = os.path.join(
            location,
            data_response.headers["content-disposition"]
            .split(";")[1]
            .split("=")[1]
            .split(".")[0],
        )
    ...
    def get_lastest_data(self, category: str, symbol: str) -> dict:
        """
        ...
        """
        data_response = self._get_data_response(
            category, symbol, get_historical_data=False
        )
